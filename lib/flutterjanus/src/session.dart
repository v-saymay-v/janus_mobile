...
class Session {
  ...
  Map<RTCPeerConnection, bool> _setRemoteDescription = {};
  ...
  Session(this.gatewayCallbacks) {
    ...
    if (Janus.isArray(this.server)) {
      ...
      if(this.server.indexOf("ws") == 0) {
        ...
        Janus.log("Using WebSockets to contact Janus: " + this.servers[this.serversIndex]);
      } else {
  ...
  createSession({GatewayCallbacks callbacks, bool reconnect = false}) {
    ...
    if (this.websockets) {
      this.ws = WebSocketWrapper(this.server, this.protocols, this.keepAlivePeriod);

      // Attach websocket handlers
      this.ws.onError = (int code, String reason) {
        Janus.error(
            "Error connecting to the Janus WebSockets server... " + servers[serversIndex]);
    ...
    httpCallbacks.success = (json) {
      ...
      if (json["janus"] != "success") {
        ...
        callbacks.error(json["error"]==null?'':json["error"]["reason"]);
        return;
      }
      this.connected = true;
      if (json["session_id"] != null) {
  ...
  destroySession(
    ...
    if (this.sessionId == null) {
      ...
      if (gatewayCallbacks != null && gatewayCallbacks.success is Function)
        gatewayCallbacks.success(null);
      if (notifyDestroyed)
        if (gatewayCallbacks != null && gatewayCallbacks.destroyed is Function)
    ...
    if (cleanupHandles) {
      ...
      this.pluginHandles.forEach((handleId, handle) {
        ...
        handle.detach(null/*callbacks*/);
      });
    }
    if (!this.connected) {
      Janus.warn("Is the server down? (connected=false)");
      this.sessionId = null;
      if (gatewayCallbacks != null)
        gatewayCallbacks.success(null);
      return;
    }
  ...
  sendMessage(int handleId, Callbacks callbacks) {
    ...
    httpCallbacks.success = (json) {
      ...
      // If we got here, the plugin decided to handle the request asynchronously
      if (callbacks.success is Function) callbacks.success(null);
    };
  ...
  destroyHandle(int handleId, Callbacks callbacks) {
    var noRequest = (callbacks != null && callbacks.noRequest == true);
    ...
    if (pluginHandle.handleToken != null)
      request["token"] = pluginHandle.handleToken;
    if (this.apiSecret != null) request["apisecret"] = this.apiSecret;
    if (this.websockets != null && this.ws != null) {
      request["session_id"] = this.sessionId;
      request["handle_id"] = handleId;
    ...
    if (pluginHandle.myStream == null ||
      ...
      if (((!media['update'] && isAudioSendEnabled(media)) ||
        ...
        if (Janus.unifiedPlan) {
          ...
          List transceivers = await pluginHandle.pc.transceivers;
    ...
    if (pluginHandle.pc == null) {
      ...
      pluginHandle.pc.onIceCandidate = (RTCIceCandidate candidate) {
        if (candidate == null ||
          ...
          if (pluginHandle.trickle == true && candidate != null) {
            // Send candidate
      ...
      if (jsep == null) {
        ...
        //if (!_setRemoteDescription.keys.contains(pluginHandle.pc)) {
        if (pluginHandle.pc.signalingState != RTCSignalingState.RTCSignalingStateStable) {
          pluginHandle.pc.setRemoteDescription(jsep).then((void v) {
            _setRemoteDescription[pluginHandle.pc] = true;
  ...
  createOffer(int handleId, Map<String, dynamic> media, Callbacks callbacks) async {
    Plugin pluginHandle = this.pluginHandles[handleId.toString()];
    if (pluginHandle == null) {
      Janus.warn("Invalid handle");
      callbacks.error("Invalid handle");
    ...
    if (Janus.unifiedPlan) {
      ...
      var transceivers = await pluginHandle.pc.transceivers;
      if (transceivers != null && transceivers.length > 0) {
        for (var t in transceivers) {
          if ((t.sender != null &&
                  t.sender.track != null &&
    ...
    if (sendVideo &&
      ...
      List<RTCRtpSender> senders = await pluginHandle.pc.senders;
      var sender = senders.firstWhere((RTCRtpSender s) => s.track.kind == 'video');
      if (sender != null) {
