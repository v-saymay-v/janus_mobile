...
class _EditScreenState extends State<EditScreen> {
  ...
  bool flagMonday = false;
  bool flagTuesday = false;
  bool flagWednesday = false;
  bool flagThursday = false;
  bool flagFriday = false;
  ...
  void initState() {
    ...
    meetingType = widget.event.meetingType;
    meetingPassword = widget.event.meetingPass;
    ...
    setState(() {
      googleId = widget.event.googleId;
      meetingId = widget.event.usePersonalMeetingId?'個人的':'自動的';
      needMeetingPass = widget.event.useMeetingPass?'必要':'不要';
      textControllerPassword = TextEditingController(text: widget.event.meetingPass);
      videoHostOn = widget.event.videoHostOn;
      videoAttendeeOn = widget.event.videoAttendeeOn;
      muteUponEntry = widget.event.muteUponEntry;
      autoRecordLocal = widget.event.autoRecordLocal;
      isRepeat = widget.event.recurrence.freqMode!=FreqMode.FreqNone;
      dailyInterval = widget.event.recurrence.dailyInterval;
      weeklyInterval = widget.event.recurrence.weeklyInterval;
      var dotw = widget.event.recurrence.weeklyDotw.split(',');
      for (String day in dotw) {
        switch (day) {
          case 'mon':
            flagMonday = true;
            break;
      ...
      monthlyInterval = widget.event.recurrence.monthlyInterval;
      if (monthlyInterval == 0)
        monthlyInterval = 1;
      monthlyBy = widget.event.recurrence.monthlyBy==MonthlyBy.byMonthDay?'毎月':'隔週';
      monthlyDay = widget.event.recurrence.monthlyDay;
      monthlyWeekIndex = widget.event.recurrence.monthlyWeekdayIndex;
      monthlyWeekDay = dayNames[DateTime.now().weekday];
      for (var i = 0; i < engNames.length; ++i) {
        if (widget.event.recurrence.monthlyWeekDay == engNames[i]) {
          monthlyWeekDay = dayNames[i];
          break;
        }
      }
      endCondition = widget.event.recurrence.endBy==EndBy.endByDate?'期限':'回数';
      var formatter = new DateFormat('yyyy-MM-dd HH:mm:ss', "ja_JP");
      textControllerEndDate = TextEditingController(text: widget.event.recurrence.endDate!=null?formatter.format(widget.event.recurrence.endDate):'');
      endTimes = widget.event.recurrence.endTimes;
    });
  ...
  Future readMemberList() async {
    ...
    if (sess != null) {
      ...
      for (int i = 0; i < mt.length; ++i) {
        ...
        widget.event.attendeeInfo.forEach((element) {
          if (meeting['userid'] == element['userid'] && element['isguest'] == 0) {
            setState(() {
              calendar.EventAttendee eventAttendee = calendar.EventAttendee();
              eventAttendee.id = element['userid'].toString();
              eventAttendee.additionalGuests = 0;
              eventAttendee.email = element['email'];
              eventAttendee.displayName = element['username'];
              attendeeEmails.add(eventAttendee);
            });
          }
        });
                      ...
                      itemBuilder: (context, index) {
                        return Padding(
                          ...
                          child: Row(
                            ...
                            children: [
                              Text(
                                attendeeEmails[index].displayName+'('+attendeeEmails[index].email+')',
                                style: TextStyle(
                              ...
                              IconButton(
                                ...
                                onPressed: () {
                                  if (attendeeEmails[index].additionalGuests > 0 && attendeeEmails[index].id != 'guest') {
                                    deleteGuest(attendeeEmails[index].id);
                                  }
                                  setState(() {
                        ...
                        IconButton(
                          ...
                          onPressed: () {
                            ...
                            if (globals.validateEmail(currentEmail) == null) {
                              setGuestInfo(currentEmail).then((guest) {
                                setState(() {
                                  if (guest != null) {
                                    attendeeEmails.add(guest);
                                  }
                              ...
                              if (endTimeInEpoch - startTimeInEpoch > 0) {
                                if (_validateTitle(currentTitle) == null) {
                                  ...
                                  if (addToGoogleCalendar) {
                                    if (googleId != null && googleId.isNotEmpty) {
                                      await calendarClient.modify(
                                        ...
                                        await storage
                                          ...
                                          .whenComplete(() => Navigator.of(context).pop(eventInfo.toJson()))
                                          .catchError((e) => print(e),
                                        );
                                      }).catchError(
                                        (e) => print(e),
                                    ...
                                    if (googleId != null && googleId.isNotEmpty) {
                                      await calendarClient.delete(googleId, shouldNofityAttendees);
                                    }
                                    await storage
                                      .updateEventData(eventInfo)
                                      .whenComplete(() => Navigator.of(context).pop(storage.lastMeetings))
  ...
  Future deleteGuest(String id) async {
    ...
    final uri = 'https://room.yourcompany.com/janusmobile/deleteguest.php';
    var map = new Map<String, dynamic>();
    map['userid'] = id;
    map['token'] = session;
    http.Response response = await http.post(Uri.parse(uri), body: map);
      ...
      setState(() {
        for (final member in _members) {
          if (member['selected']) {
            ...
            attendee.id = (member['userid']).toString();
  ...
  Future<calendar.EventAttendee> setGuestInfo(String email) async {
    String organization = "";
    String guestName = "";
    ...
    return await showDialog<calendar.EventAttendee>(
        context: context,
        barrierDismissible: false,
        builder: (context) {
          return StatefulBuilder(
                  ...
                  actions: <Widget>[
                    ...
                    TextButton(
                      ...
                      onPressed: () {
                        ...
                        eventAttendee.additionalGuests = 1;
                        //attendeeEmails.add(eventAttendee);
                        Navigator.of(context).pop(eventAttendee);
                      }
                    ),
                  ],
                  content: Container(
